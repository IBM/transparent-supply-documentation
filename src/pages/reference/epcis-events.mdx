...
**<SolutionName />** enables your **organization** to trace the physical
movement and status of products on your supply chain, and to make this data
available to your selected business partners. To help achieve this goal, <SolutionName />
...
## EPCIS and <SolutionName />

Using EPCIS standards, supply chain events that describe products and facilities
registered with <SolutionName /> can be uploaded to the network. This event data can
then be traced on <SolutionName /> to show end-to-end supply chain flows for specific
product instances. The following six EPCIS events are defined in <SolutionName />;
...
Uploading a commission event to <SolutionName /> creates the product instance on the
...
farm that is not yet defined to <SolutionName />, the batch would not be commissioned
until it arrives at a processing facility that is registered with <SolutionName />.
Each instance of an end-to-end supply chain starts with a commission event that
instantiates, or creates, a new product instance on <SolutionName />.
...
before they are uploaded to <SolutionName /> and reflected in product traces.
...
<p>
  <SolutionName /> supports uploading EPCIS events in the GS1 standardized XML format. Annotated EPCIS XML files are provided, which contain all available elements and comments for each event type. You can use these XML files as templates to generate your own EPCIS event files for uploading to <SolutionName />.
</p>

EPCIS events can be uploaded to <SolutionName /> using either of the following methods:
...
2. Using spreadsheets:
  ...
  c. Upload the data using the spreadsheet bulk upload page in the <SolutionName /> UI.
