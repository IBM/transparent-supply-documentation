...
Once your **organization** has joined <SolutionName />, new human users can only be added
...
To successfully log in to <SolutionName />, each human user must have an IBMid that is
registered with **<SolutionName />**. One of the following scenarios will apply:

1. If you **do not have an IBMid** (or would like an additional IBMid for <SolutionName />
 login): [**Sign up for an IBMid**](#sign-up-for-an-ibmid), ensuring that your <SolutionName />
...
1. If you **already have an IBMid that is registered with <SolutionName />**: No further
steps are required&mdash;you can proceed to <a href={process.env.GATSBY_UI_URL + '/'}>**<SolutionName />**</a> and log in with
your **existing IBMid and password**.
1. You **have an IBMid that is NOT registered with <SolutionName />**: Either: a) Your
...
address that is also your <SolutionName /> login email address (see below).

## Sign up for an IBMid

If you do not have a registered IBMid that is registered with <SolutionName />, create a
new (or additional) [**IBMid**](https://www.ibm.com/account/us-en/signup/register.html) using the
following steps. Your new IBMid **MUST** be the email address that is registered with <SolutionName />:

1. Open your <SolutionName /> new user login email message (if applicable) and click on the link
...
page, complete the **Sign up for an IBMid** form. **IMPORTANT:** Your <SolutionName /> registration
...
4. You are now ready to log in to <SolutionName />! Proceed to <a href={process.env.GATSBY_UI_URL + '/manageUsers'}>
**<SolutionName />**</a> and log in with your new IBMid and password.
