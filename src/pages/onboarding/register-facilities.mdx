...
[custom identifier](/reference/solution-identifiers). <SolutionName /> supports
...
and is used for registering locations (facilities) on <SolutionName />.
...
* Text description
* GLN or [<SolutionName /> facility ID](/reference/solution-identifiers)
...
Connector API</a> to register your products with <SolutionName />. You must authenticate first to
...
individual to <SolutionName />.

**Attention**: Clicking the Swagger **Try it out** button submits your API call to the <SolutionName /> network
...
- Refer to the supported schema and example XML [data types](/reference/data-types) as needed. <SolutionName />
verifies the submitted XML with the corresponding schema. The response will describe any errors.
- Review your data entitlement and access protocols for details on sharing data with your <SolutionName />
...
1. Click the **Try it Out** button. If successful, the data will upload to <SolutionName /> with a response code of "201". A response of
